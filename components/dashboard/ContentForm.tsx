...
const S = {
  ...
  input: { width: '100%', background: '#111', border: '1px solid #2a2a2a', borderRadius: 6, padding: '10px 14px', color: '#f0ede6', fontSize: 14, outline: 'none', fontFamily: 'inherit' },
  row: { marginBottom: 18 },
          ...
          <div key={f.name} style={S.row}>
            {f.type === 'image' ? (
              <ImageUpload
                ...
                folder={f.imageFolder || 'sahab-portfolio'}
              />
            ) : f.type === 'textarea' ? (
              <>
              ...
              <>
                <label style={S.label}>{f.label}{f.required && ' *'}</label>
                <select value={values[f.name] || ''} onChange={(e) => set(f.name, e.target.value)} style={S.input}>
              ...
              <>
                ...
                <input
                  type="text"
                  placeholder={f.placeholder || 'Type and press Enter to add'}
                  style={S.input}
                  onKeyDown={(e) => {
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
                  {(values[f.name] || []).map((tag: string, i: number) => (
                    <span
                      key={i}
                      onClick={() => set(f.name, values[f.name].filter((_: any, j: number) => j !== i))}
              ...
              <>
                <label style={S.label}>{f.label}{f.required && ' *'}</label>
                <input type="number" value={values[f.name] ?? ''} onChange={(e) => set(f.name, Number(e.target.value))} required={f.required} style={S.input} />
              </>
            ) : (
              <>
                <label style={S.label}>{f.label}{f.required && ' *'}</label>
