...
export default function PortfolioFilter({ projects }: { projects: Project[] }) {
  ...
  const handleFilter = async (val: string) => {
    ...
    await gsap.to(items, { opacity: 0, scale: 0.94, duration: 0.2, ease: 'power2.in' })
    setActive(val)
    setAnimating(false)
    setTimeout(async () => {
  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', padding: '26px 0', borderBottom: '1px solid var(--border)' }}>
        {FILTERS.map((f) => (
          <button
            key={f.value}
